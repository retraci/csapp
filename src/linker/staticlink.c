...
static void R_X86_64_32_handler(elf_t *dst, sh_entry_t *sh, int row_referencing, int col_referencing, int addend,
                                st_entry_t *sym_referenced);
static void R_X86_64_PC32_handler(elf_t *dst, sh_entry_t *sh, int row_referencing, int col_referencing, int addend,
                                  st_entry_t *sym_referenced);

typedef void (*rela_handler_t)(elf_t *dst, sh_entry_t *sh, int row_referencing, int col_referencing, int addend,
                               st_entry_t *sym_referenced);
...
void link_elf(elf_t **srcs, int num_srcs, elf_t *dst) {
    ...
    for (int i = 0; i < smap_count; i++) {
        ...
        debug_printf(DEBUG_LINKER, "%s\t%d\t%d\t%s\t%d\t%d\n",
                     ste->st_name, ste->bind, ste->type, ste->st_shndx, ste->st_value, ste->st_size);
    }
...
static void symbol_processing(elf_t **srcs, int num_srcs, elf_t *dst, smap_t *smap_table, int *smap_count) {
    ...
    for (int i = 0; i < num_srcs; i++) {
        ...
        for (int j = 0; j < elf->symt_count; j++) {
            st_entry_t *sym = &(elf->symt[j]);
            ...
            if (sym->bind == STB_LOCAL) {
                ...
                for (int k = 0; k < *smap_count; k++) {
                    ...
                    if (candidate->bind == STB_GLOBAL && (strcmp(candidate->st_name, sym->st_name) == 0)) {
                        ...
                        simple_resolution(sym, elf, &smap_table[k]);
