...
static void parse_instruction(const char *str, inst_t *inst) {
    ...
    int len = strlen(str);
    for (int i = 0; i < len; i++) {
        if (str[i] == '(' || str[i] == ')') count_parentheses++;
        ...
        if (state == 1) op_str[op_len++] = str[i];
        else if (state == 3) src_str[src_len++] = str[i];
        else if (state == 5) dst_str[dst_len++] = str[i];
    }
...
static void parse_operand(const char *str, od_t *od) {
    ...
    if (str[0] == '$') {
        ...
        od->imm = string2uint_range(str, 1, end);
    } else if (str[0] == '%') {
        // register
        od->type = REG;
        ...
        // parse reg1
        if (reg1_len > 0) od->reg1 = try_get_from_trie(&register_mapping, reg1);
        // parse reg2
        if (reg2_len > 0) od->reg2 = try_get_from_trie(&register_mapping, reg2);
...
static void sub_handler(od_t *src_od, od_t *dst_od) {
    ...
    if (src_od->type == IMM && dst_od->type == REG) {
        ...
        cpu_flags.ZF = (val == 0);
        cpu_flags.SF = val_sign;
        cpu_flags.OF = (src_sign == 1 && dst_sign == 0 && val_sign == 1) || (src_sign == 0 && dst_sign == 1 && val_sign == 0); // singed
...
void print_register() {
    if ((DEBUG_VERBOSE_SET & DEBUG_REGISTERS) == 0x0) return;
...
void print_stack() {
    if ((DEBUG_VERBOSE_SET & DEBUG_PRINTSTACK) == 0x0) return;
