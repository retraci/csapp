...
static void TestAddFunctionCallAndComputation() {
    ...
    for (int i = 0; i < 15; i++) {
        writeinst_dram(va2pa(i * MAX_INSTRUCTION_CHAR * sizeof(char) + 0x00400000), assembly[i]);
    }
    int start = 11;
    ...
    if (match) printf("register match\n");
    ...
    if (match) printf("memory match\n");
    else printf("memory mismatch\n");
}
...
static void TestSumRecursiveCondition() {
    ...
    printf("begin\n");
    int time = 0;
    while ((cpu_pc.rip <= 18 * MAX_INSTRUCTION_CHAR * sizeof(char) + 0x00400000) && time < MAX_NUM_INSTRUCTION_CYCLE) {
        instruction_cycle();
        print_register();
        print_stack();
        time++;
