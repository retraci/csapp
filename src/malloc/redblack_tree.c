...
static int set_redblack_tree_parent(uint64_t header_vaddr, uint64_t prev_vaddr) {
    set_field32_block_ptr(header_vaddr, prev_vaddr, MIN_REDBLACK_TREE_BLOCKSIZE, 4);
    return 1;
}

static int set_redblack_tree_left(uint64_t header_vaddr, uint64_t left_vaddr) {
    set_field32_block_ptr(header_vaddr, left_vaddr, MIN_REDBLACK_TREE_BLOCKSIZE, 8);
    return 1;
}

static int set_redblack_tree_right(uint64_t header_vaddr, uint64_t right_vaddr) {
