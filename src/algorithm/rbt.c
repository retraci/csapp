...
static uint64_t rbt_internal_rotate(uint64_t n, uint64_t p, uint64_t g, rbtree_internal_t *tree, rbtree_node_interface *i_node) {
    assert(tree != NULL);
...
static void rbt_get_psnf(rbtree_internal_t *tree, rbtree_node_interface *i_node, uint64_t db,
    ...
    // this calculation is current for (db == NULL) case
